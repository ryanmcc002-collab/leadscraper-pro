...
(function (root) {
  ...
  let toastTimer = null;
  // A tiny self-contained page that saves the REAL file when opened.
  // claude.ai's save prompt allows .html but not .pdf/.dxf, so the file
  // travels inside this carrier and downloads itself with the proper name
  // the moment the user opens it — outside the sandbox, where downloads work.
  function carrierHTML(filename, text, type) {
    // UTF-8 safe base64 (DXF labels can contain Chinese)
    let b64;
    try { b64 = btoa(unescape(encodeURIComponent(text))); }
    catch (e) { b64 = btoa(text); }
    const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;');
    return '<!doctype html><html><head><meta charset="utf-8"><title>Save ' + esc(filename) + '</title></head>' +
      '<body style="font-family:-apple-system,Segoe UI,Arial,sans-serif;display:grid;place-items:center;height:100vh;margin:0;background:#eef1f4;color:#21303e">' +
      '<div style="text-align:center;background:#fff;border:1px solid #d8dee6;border-radius:12px;padding:36px 44px">' +
      '<div style="font-weight:700;letter-spacing:.4px;margin-bottom:14px">BONDI <span style="color:#1f7a35">TRAILER CAD</span></div>' +
      '<p style="margin:0 0 18px">Your file should be downloading now. If not, click:</p>' +
      '<a id="dl" download="' + esc(filename) + '" style="font-size:19px;background:#1f7a35;color:#fff;padding:12px 22px;border-radius:8px;text-decoration:none">&#128190; Save ' + esc(filename) + '</a>' +
      '</div><script>' +
      'var b="' + b64 + '";var s=atob(b);var u8=new Uint8Array(s.length);' +
      'for(var i=0;i<s.length;i++)u8[i]=s.charCodeAt(i);' +
      'var url=URL.createObjectURL(new Blob([u8],{type:"' + type + '"}));' +
      'var a=document.getElementById("dl");a.href=url;setTimeout(function(){a.click();},300);' +
      '<\/script></body></html>';
  }
  ...
  async function download(filename, content, mime) {
    ...
    if (window.claude && window.claude.downloads) {
      ...
      try {
        ...
        // claude.ai's allowlist has no .pdf/.dxf. Wrap text files in a small
        // .html carrier that saves the real file when opened — no renaming.
        if (typeof content === 'string') {
          try {
            await window.claude.downloads.save({ filename: filename + '.html', data: carrierHTML(filename, content, type) });
            noteToast('✓ Saved "' + filename + '.html" — claude.ai can\'t save this file type directly. ' +
              'Just open that file (double-click it) and the real ' + filename + ' downloads itself, correctly named.');
            return;
          } catch (e15) {
            if (e15 && e15.code === 'declined') return;
          }
        }
        // last resort: same bytes with a .txt ending (needs a manual rename)
        try {
